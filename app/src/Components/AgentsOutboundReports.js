...
export default function AgentsOutboundReports() {
  ...
  const getData = (e) => {
    if(number.length > 3){
      return axios.get(API_URL + `/api/voice/agents/reports?sort=-updatedAt&type=outbound&calleridnum=${number}&limit=25`, {
          headers: {
    ...
    return axios.get(API_URL + "/api/voice/agents/reports?sort=-updatedAt&type=outbound&limit=25", {
        headers: {
            'Authorization': 'Basic ' + btoa(`${name}:${password}`)
        }
    }
  ...
  return (
      <div>
          <h2>Get Agents Outbound Reports</h2>
          <form>
